...
class FungsiUmum
{
	...
	public function cekTokenValid($token)
	{
		$tokensession = isset($_SESSION["u_token"]) ? $_SESSION["u_token"] : '';

		if ($token == $tokensession) return true;
		else return false;
	}

	function cekTokenValid2()
	{
		$token = isset($_GET['u_token']) ? $_GET['u_token'] : '';
		$user_login = isset($_SESSION["userlogin"]) ? $_SESSION["userlogin"] : '';
		$u_token = isset($_SESSION["u_token"]) ? $_SESSION["u_token"] : '';

		if ($user_login != '' && $token != '' && $u_token != '') {
			if ($token == $u_token) {
				return true;
			} else {
				return false;
	...
	public function cetakcombobox4($juduloption, $ukuran, $id = 0, $idobject, $tabel, $fieldoption, $fieldisi, $kelascss, $readonly, $where = Null)
	{
		$sql = 'select ' . $fieldoption . ' as opt,' . $fieldisi . ' as value from ' . $tabel;
		if ($where != Null || $where != '') $sql .= ' WHERE ' . $where;
		...
		$createcombo = '<select id="' . $idobject . '" name="' . $idobject . '"' . $kelascss . $style . '' . $readonly . '>';
		if ($juduloption != '') $createcombo .= '<option value="0">' . $juduloption . '</option>';
		foreach ($query->rows as $rs) {
			$fieldsopt = explode(" as ", $fieldoption);
			$fieldoptnya = isset($fieldsopt[1]) ? $fieldsopt[1] : $fieldoption;
	...
	public function fFormatuang($rp)
	{ //format uang rupiahs
		return 'Rp. ' . number_format($rp, 2, ',', '.');
	}
	public function fuang($rp)
	{ //format uang rupiahs
	...
	public function cekHak($folder, $aksi, $methodAjax)
	{
		$error = false;
		$menu = $folder . $aksi;
		$session_menu = isset($_SESSION[$menu]) ? $_SESSION[$menu] : 0;
		//$session_masuk = isset($_SESSION["masukadmin"]) ? $_SESSION["masukadmin"] : '';
		//if ($session_masuk != "xjklmnJk1o~") $error = true;
		//if (isset($_SESSION[$menu]) != 1) $error = true;
		if ($session_menu != 1) $error = true;
		if ($methodAjax != 1) {
	...
	public function UploadImagebyUkuran($fotonya, $nmfileasli, $nmfilenya, $dimensi, $dimensi2 = Null,$target='_other')
	{
		...
		if (!move_uploaded_file($fotonya, $vfile_upload)) {
			...
			switch ($size[2]) {
				case 1:       //GIF
					...
					Imagegif($image_new, DIR_IMAGE . $target ."/other_" . $nmfilenya);
					break;
				case 2:       //JPEG
					//imagejpeg($im,DIR_IMAGE."_other/other_".$nmfilenya,100);
					imagejpeg($image_new, DIR_IMAGE . $target ."/other_" . $nmfilenya, 100);
					break;
				case 3:       //PNG
					//ImagePNG($im,DIR_IMAGE."_other/other_".$nmfilenya); 
					ImagePNG($image_new, DIR_IMAGE . $target ."/other_" . $nmfilenya);
					break;
				default:
					return false;
					break;
	...
	public function ftanggalFull2($tanggal)
	{ //format tanggal dari yyyy-mm-dd H:i:s menjadi dd/mm/yyyy
		$xt = "";
		$xt = explode(" ", $tanggal);
		$jam = $xt[1];
	...
	public function ftanggal2($tanggal)
	...
	public function ftanggal1($tanggal)
	{ //format tanggal dari dd/mm/yyyy menjadi yyyy-mm-dd
		$xt = "";
		$xt = explode("/", $tanggal);
		$tgl = $xt[0];
		$bln = $xt[1];
	...
	public function ftanggal3($tanggal)
	{ //format tanggal dari yyyy/mm/dd menjadi yyyy-mm-dd
		$tgl = str_replace("/", "-", $tanggal);
		return $tgl;
	}
	public function ftanggalFull1($tanggal)
	{ //format tanggal dari yyyy-mm-dd H:i:s menjadi dd/mm/yyyy H:i:s
		$xt = "";
		$xt = explode(" ", $tanggal);
		$jam = $xt[1];
		$tgl = self::ftanggal2($xt[0]);
	...
	function grabData($data)
	{
		...
		curl_setopt_array($data['curl'], array(
			CURLOPT_URL => $data['urlcurl'],
			...
			CURLOPT_HTTPHEADER => $data['httpheader'],
		));

		return curl_exec($data['curl']);
	}
	function __destruct()
	{
		$this->db->disconnect();
