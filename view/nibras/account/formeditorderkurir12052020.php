				...
				<div class="form-group">
					...
					<select class="form-control form-control-sm" name="serviskurir" id="serviskurir" onchange="cektarifkurir()">
						...
						<?php foreach($formkurir['servis'] as $ship) {?>
							...
							<?php } else { ?>
							<option value="<?php echo $ship['servis_id'] ?>::Konfirmasi Admin::<?php echo $ship['shipping_kode'] ?>::<?php echo $ship['servis_code'] ?>"><?php echo $ship['shipping_kode'].' - '.$ship['servis_code'].' - '.$ship['hrg_perkilo'] ?></option>
							<?php } ?>
						<?php } ?>
						<?php } ?>
					<select>
