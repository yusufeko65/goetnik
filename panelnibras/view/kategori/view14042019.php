   ...
   <div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['kategori_id']?>" /></td>
		   <td><?php echo stripslashes($datanya["kategori_nama"]) ?></td>
		   <td><?php echo $datanya["kategori_urutan"]?></td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['kategori_id']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
