...
<div class="col-lg-12 main-content">
	<h2 class="judulmodul"><?php echo $judul ?> <font color="green"> <?php echo $_SERVER['HTTP_HOST'] ?> </font> </h2>
	<div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
		<thead>
			<tr>
				...
				<td>
					<a href="<?php echo $orderurl . "/?sort=" . $sort['id'] . "&u_token=" . $u_token ?>">
						Order ID <i class="glyphicon <?php print($icon['id']) ;?>"></i>
					</a>
				</td>
				<td>
					<a href="<?php echo $orderurl . "/?sort=" . $sort['customer'] . "&u_token=" . $u_token ?>">
						Customer <i class="glyphicon <?php print($icon['customer']) ;?>"></i>
					</a>
				</td>
				<td>Penerima</td>
				...
				<td class="text-center">
					<a href="<?php echo $orderurl . "/?sort=" . $sort['date'] . "&u_token=" . $u_token ?>">
						Tgl <i class="glyphicon <?php print($icon['date']) ;?>"></i>
					</a>
				</td>
				<td><?php print($lblStatus) ;?></td>
		...
		<tbody id="viewdata">
			...
			<?php foreach ($ambildata as $datanya) { ?>
				...
				<tr <?php echo $kelasgrid ?>>
					...
					<td class="text-right"><?php echo $dtFungsi->fFormatuang($datanya["pesanan_kurir"] + $datanya["subtotal"] - $datanya['dari_poin'] - $datanya['kode_unik'] + $datanya['biaya_packing']) ?></td>
