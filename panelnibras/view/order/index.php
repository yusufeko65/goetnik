...
$orderurl = URL_PROGRAM_ADMIN . folder;
...
switch ($menupage) {
	...
	default:
		...
		$sort = array(
			'id'		=> 'pesanan_no-asc',
			'date'		=> 'pesanan_tgl-asc',
			'customer'	=> '_customer.cust_nama-asc',
			'status'	=> 'status_id-asc'
		);

		$icon = array(
			'id'		=> 'glyphicon-sort-by-attributes-alt',
			'date'		=> 'glyphicon-sort',
			'customer'	=> 'glyphicon-sort',
			'status'	=> 'glyphicon-sort'
		);

		if(isset($_GET['sort'])){
			$sort = array(
				'id'		=> 'pesanan_no-desc',
				'date'		=> 'pesanan_tgl-desc',
				'customer'	=> '_customer.cust_nama-desc',
				'status'	=> 'status_id-desc'
			);

			$icon['id'] = 'glyphicon-sort';

			switch ($_GET['sort']) {
				case 'pesanan_no-desc':
					$sort['id'] = 'pesanan_no-asc';
					$icon['id'] = 'glyphicon-sort-by-attributes-alt';
					break;

				case 'pesanan_tgl-desc':
					$sort['date'] = 'pesanan_tgl-asc';
					$icon['date'] = 'glyphicon-sort-by-attributes-alt';
					break;

				case '_customer.cust_nama-desc':
					$sort['customer'] = '_customer.cust_nama-asc';
					$icon['customer'] = 'glyphicon-sort-by-attributes-alt';
					break;

				case 'status_id-desc':
					$sort['status'] = 'status_id-asc';
					$icon['status'] = 'glyphicon-sort-by-attributes-alt';
					$func = 'date_dsort';
					break;

				case 'pesanan_no-asc':
					$icon['id'] = 'glyphicon-sort-by-attributes';
					break;

				case 'pesanan_tgl-asc':
					$icon['date'] = 'glyphicon-sort-by-attributes';
					break;

				case '_customer.cust_nama-asc':
					$icon['customer'] = 'glyphicon-sort-by-attributes';
					break;

				case 'status_id-asc':
					$icon['status'] = 'glyphicon-sort-by-attributes';
					$func = 'date_sort';
					break;
			}
		}

		if(isset($_GET['datacari']) || isset($_GET['status'])){
			 $_dtcari = "&datacari=".$_GET['datacari']."&status=".$_GET['status'];
			 $sort['id'] .= $_dtcari;
			 $sort['date'] .= $_dtcari;
			 $sort['customer'] .= $_dtcari;
			 $sort['status'] .= $_dtcari;
		}
		...
		if(isset($_GET['status']) && !empty($_GET['status'])){
			...
			$lblStatus = '<a href="'.$orderurl . '/?sort=' . $sort['status'] . '&u_token=' . $u_token .'">
							Status <i class="glyphicon '.$icon['status'].'"></i>
						</a>';

			if(!empty($func)){
				$dsort = array();
				foreach ($ambildata as $key => $datanya) {
					...
					$dsort[$key] = isset($tanggal_status['tanggal']) ? $tanggal_status['tanggal'] : '';
				}

				uasort($dsort, $func);	
				$tmpdata = $ambildata;
				$ambildata = array();
				foreach ($dsort as $key => $val) {
					$ambildata[] = $tmpdata[$key];
				}
			}
		}
