<form method="POST" name="frmaddproduk" id="frmaddproduk" action="<?php echo $_SERVER['PHP_SELF'] ?>">
	...
	<input type="hidden" id="produk_id" name="produk_id" value="<?php echo $produkid ?>">
	...
	<input type="hidden" name="urlfolder" id="urlfolder" value="<?php echo URL_PROGRAM_ADMIN.folder.'?op=info&pid='.$pesanan_no ?>">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<div class="form-group">
				   <button type="button" id="btnsimpanaddproduk" onclick="simpanaddproduk()" class="btn btn-sm btn-success">Simpan</a>
...
$(function(){
	...
	$('#idwarna').chosen().change(function() {
		var action = $('#frmaddproduk').attr("action");
		var url = action + '?modul=stokbywarnaukuran';
		var idproduk = $('#produk_id').val();
...
function simpanaddproduk(){
	...
	var url = $('#frmaddproduk').prop("action")+'?modul=addorderproduk';
	var qty = $('#qty').val();
