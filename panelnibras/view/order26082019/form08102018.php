				...
				<div class="col-md-12">
					<div class="row">
						...
						<div id="platcari" class="alert alert-info">
							<input type="text" id="cariproduk" name="cariproduk" placeholder="Cari Nama produk yang ingin di tambah" class="form-control input-lg" autocomplete="off">
						</div>
					</div>
				</div>
	...
	$('#cariproduk').autocomplete({
		...
		select: function( event, ui ) {
			...
			var zdata = ui.item.id+":"+ui.item.kode+":"+
						...
						nopesan+':'+grppelanggan+':'+pelangganid;
			$("#loadingweb").fadeIn();
			tampilform(url,zdata);
			return false;
		},
...
function formAlamat(jenis){
	...
	var zdata = nopesan+'::'+pelangganid+'::'+grppelanggan+'::'+jenis+'::'+caption_alamat+'::'+totberat;
	$("#loadingweb").fadeIn();
	if(grup_dropship == '0' ) {
...
function hapusprodukorder(){
	var datanya = "produk_id="+$('#didproduk').val()+
