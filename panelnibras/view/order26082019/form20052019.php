		...
		<div class="widget-content nopadding">
			<form autocomplete="off" method="POST" name="frmdata" id="frmdata" action="<?php echo $_SERVER['PHP_SELF'] ?>" class="form-horizontal">
				...
				<div class="col-md-12 text-right">
					...
					?>
					<a href="<?php echo URL_PROGRAM_ADMIN . folder . '?modul=cetaklabel&pid=' . $iddata ?>" target="_blank" id="btncetaklabel" class="btn btn-sm <?php echo $class_button ?>"><i class="icon-print" aria-hidden="true"></i> Cetak Label</a>
				</div>
				...
				<div class="well plat-order">
					...
					<div class="col-md-4">
						...
						<?php if ($order['status_id'] != $dataset['config_ordercancel']) { ?>
							<a href="javascript:void(0)" id="btnEditStatus" class="btn btn-xs btn-primary"><span class="glyphicon glyphicon-pencil"></span> Edit</a>
						<?php } else { ?>
							<?php
							$tabeljoin = "`_order_status` LEFT JOIN _login ON _order_status.admin_id = _login.login_id";
							$wherejoin = "nopesanan='" . $order['pesanan_no'] . "' AND status_id = '" . $dataset['config_ordercancel'] . "'";
							$canceledby = $dtFungsi->fcaridata2($tabeljoin, 'login_nama', $wherejoin);
							if ($canceledby['login_nama'] != null) {
								echo ' (' . $canceledby['login_nama'] . ')';
							} else {
								echo ' (Sistem)';
							}
							?>
...
<script>
	...
	function editDeposito() {
		var url = action + '?modul=frmEditDeposito';
	...
	function simpaneditdeposito() {
		...
		var url = $('#frmdata').prop("action") + '?modul=simpaneditpotongandeposit';
		//alert(url+datanya);
		//return false;
		$.ajax({
			type: "POST",
	...
	function updateketerangan() {
		...
		var data = 'keterangan=' + $('#keterangan').val() + '&tampilketerangan=' + tampilketerangan + '&pesanan_no=' + $('#iddata').val();
		var url = $('#frmdata').prop("action") + '?modul=updateketeranganorder';
