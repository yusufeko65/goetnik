			...
			<form autocomplete="off" method="POST" name="frmdata" id="frmdata" action="<?php echo $_SERVER['PHP_SELF'] ?>" class="form-horizontal">
				...
				<div class="well plat-order">
					...
					<div class="col-md-4">
						...
						if ($order['status_id'] == $dataset['config_ordercancel']  || $order['status_id'] == $dataset['config_shippingstatus']) {
				...
				<table class="table table-bordered table-striped tabel-grid">
					...
					<tbody>
						...
						foreach ($datadetail as $dt) {
							...
							if ($harga_tambahan) {
								$harga_normal .= '<br><small> + ' . $dtFungsi->fFormatuang($harga_tambahan) . ' <br>(tambahan harga size) </small>';
							}
...
<script>
	...
	$(function() {
		...
		$('#btnEditStatus').click(function() {
			...
			var zdata = nopesan + ',' + stsnow + ',' + stsshipping + ',' + stskonfirm + ',' + namaservis + ',' + tglshipping + ',' + awbshipping + ',' + stsgetpoin + ',' + pelangganid + ',' + totpoin + ',' + stssudahbayar + ',' + grandtotal + ',' + stscancel;
