<div class="col-lg-12 main-content">
	 ...
	 <div class="widget-box">
		...
		<div class="widget-content nopadding">
			<form method="POST" autocomplete="off" name="frmdata" id="frmdata" action="<?php echo $_SERVER['PHP_SELF'] ?>">
				...
				<div role="tabpanel">
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="tabproduk">
							<div class="well">
								...
								<div class="text-right">
									<a href="<?php echo URL_PROGRAM_ADMIN.folder ?>" class="btn btn-lg btn-warning">Batal</a>
						...
						<div role="tabpanel" class="tab-pane" id="tabmsgorder">
							<div class="well">
								<div id="msgorder"></div>
								<a href="<?php echo URL_PROGRAM_ADMIN.folder ?>?op=add" class="btn btn-lg btn-warning">Order Baru</a>
							</div>
						</div>
					</div>
					<!-- @end Tab panes -->
...
function servisKurir(propinsi,kota,kecamatan){
	var data = 'propinsi_penerima='+propinsi+'&kabupaten_penerima='+kota+'&kecamatan_penerima='+kecamatan;
	$.ajax({
		type: "POST",
		...
		success: function(result) {
			var hrgperkilo = 0;
			var html = '<option value="0">- Pilih Kurir -</option>';
			if(result['status'] == 'success') {
				for (i = 0; i < result['result'].length; i++) {
					hrgperkilo = result['result'][i]['hrg_perkilo'];
					if(Number.isInteger(hrgperkilo)) {
						hrgperkilo = convertToRupiah(hrgperkilo);
					} 
					
					html += '<option value="' + result['result'][i]['servis_id'] + '">';
					
					if(hrgperkilo == 'Konfirmasi Admin') {
						html += result['result'][i]['shipping_kode'] + ' - '+result['result'][i]['servis_code'] + ' '+ hrgperkilo + '</option>';
					} else {
						html += result['result'][i]['shipping_kode'] + ' - '+result['result'][i]['servis_code'] + ' ('+ result['result'][i]['keterangan'] +' Hari) - '+ hrgperkilo + '</option>';
					}
...
function gantiAlamat(jenis_alamat,nama,no_hp,alamat,propinsi,propinsi_caption,kabupaten,kabupaten_caption,kecamatan,kecamatan_caption,kelurahan,kodepos){
	...
	if(jenis_alamat == 'alamatpenerima') {
		servisKurir(propinsi,kabupaten,kecamatan);
