...
switch($module){
   ...
   case "frmEditProduk":
	 ...
	 if($data != ''){
	   ...
	   if(($uk != '' && $uk != '0') || ($idwarna != '' && $idwarna !='0')) {
		  ...
		  //$warna = $dtProduk->getProdukOrderWarnaByUkuran($produkid,$idukuran,$idwarna,$nopesan);
   ...
   case "carieditproduk":
	   ...
	   $stok = $stok == '' ? 0 : $stok;
	   //echo json_encode($dtProduk->getProdukOrderStokOption($idproduk,$idwarna,$idukuran,$nopesan));
	   //$datawu = $dtOrder->getProdukOrderOption($idproduk,$idwarna,$idukuran,$nopesan);
	   /*
	   $data['ukuran'] = $ukuran;
	   $data['warna'] = $warna;
	   $data['jmlstok'] = $jmlstok;
	   $data['wn'] = $wn;
	   $data['uk'] = $uk;
	   ...
	   foreach($datawu as $wu) {
		 ...
		 if($wu['ukuran'] != $idukuran) {
		   $uk[] = $wu['ukuran'];
		 }
	   }
	  $zukuran = array();
	  $zwarna = array();
	   foreach($ukuran as $uks){
	     if(!in_array($uks['id'], $uk) )  {
		     //$ukuran['id'][] = $uks['id'];
			 //$ukuran['nm'][] = $uks['nm'];
			 $zukuran[] = array('id'=>$uks['id'],'nm'=>$uks['nm']);
		 }
	   }
	   foreach($warna as $wns){
	      if(!in_array($wns['id'], $wn) )  {
		     //$zwarna['id'][] = $wns['id'];
			 //$zwarna['nm'][] = $wns['nm'];
			 $zwarna[] = array('id' => $wns['id'],'nm'=>$wns['nm']);
		  }
	   }
	   */
	   $data['ukuran'] = $ukuran;
       ...
       if(isset($_SESSION['hsadmincart'][$idmember])) {
		   ...
		   if($totalitem > 0){
  		   foreach($cart as $c){
			  ...
			  //$html .= '<tr><td colspan="6" class="kanan"><b>' . $dtFungsi->terbilang($subtotal) .' Rupiah</b></td></tr>';
		   } else {
...
switch($aksipage){
   ...
   case "simpanstatusorder":
     echo $dtOrder->simpanstatusorder();
     exit;
   break;
}
...
switch($menupage){
	...
	case "info": 
		...
		if(!empty($order)){
			...
			$dataalamat 	= $dtOrder->dataOrderAlamat($iddata);
			$alamatreport   = implode("::",$dataalamat);
			$kurir			= $dtOrder->getShippingbyName($order[9]);
			$tabelservis	= $kurir['tabel_servis'];
			$tabeltarif		= $kurir['tabel_tarif'];
	 	
			$servisdata    	= $dtOrder->getServisbyId($tabelservis,$order[10]);
			$namaservis		= $servisdata[1];
			$minkilo		= $servisdata[3];
			
			$whereprint		= "nopesanan=$order[1] AND status_id=$order[8]";
			...
			$where 			= "cust_id = '".$order[2]."'";
			$reseller 		= $dtFungsi->fcaridata2($tabel,$field,$where);
			$nmreseller 	= $reseller[0];
			$grupreseller	= $reseller[1];
			
			unset($_SESSION['hsadmincart']);
			unset($_SESSION['wrnadmincart']);
			unset($_SESSION['ukradmincart']);
			unset($_SESSION['qtyadmincart']);
			unset($_SESSION['katadmincart']);
