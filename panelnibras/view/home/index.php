...
$where = "setting_key IN ('config_orderstatus','config_masabayar','config_shippingstatus','config_sudahbayarstatus')";
...
$whereorderhmin1 = "_order.status_id='" . $data['config_orderstatus'] . "' AND DATE_FORMAT(((pesanan_tgl + INTERVAL ".$data['config_masabayar']." DAY) - INTERVAL 1 DAY),'%Y-%m-%d') <= '".$tanggal_sekarang."' ORDER BY pesanan_tgl ASC";
$dataorderhmin1  = $dtFungsi->fcaridata3($tabelorderhmin1, $fieldorderhmin1, $whereorderhmin1);

$dataorder['status'] = $data['config_shippingstatus'];
$dataorder['status_order_kirim'] = $data['config_shippingstatus'];
$dataorder['tanggal1'] = $tanggal_sekarang;
$dataorder['tanggal2'] = $tanggal_sekarang;
$totalorder = $m_order->getTotalOrderDaily($dataorder);



?>
<div class="col-lg-12 main-content">
	<h3 class="judulmodul">Selamat Datang di <font color="green"> <?php echo $_SERVER['HTTP_HOST'] ?> </font>, <?php echo $_SESSION['userlogin'] ?>   </h3>
		...
		<div class="col-lg-4 col-md-4 col-sm-12">
			...
			<div class="tile">
				...
				<div class="tile-body"><i class="glyphicon glyphicon-user"></i>
					<h3 class="pull-right"><?php echo $dtFungsi->fuang($datacust['total']) ?></h3>
				</div>
		...
		<div class="col-lg-4 col-md-4 col-sm-12">
			...
			<div class="tile">
				...
				<div class="tile-body"><i class="glyphicon glyphicon-shopping-cart"></i>
					<h3 class="pull-right"><?php echo $dtFungsi->fuang($dataorder['total']) ?></h3>
				</div>
				<div class="tile-footer"><a href="<?php echo URL_PROGRAM_ADMIN . 'order?u_token=' . $u_token ?>" class="link">Selengkapnya</a></div>
			</div>
		</div>
		<?php if(isset($_SESSION['lap-order']) && $_SESSION['lap-order'] == '1') { ?>
		<div class="col-lg-4 col-md-4 col-sm-12">
			<!-- plat order -->
			<div class="tile">
				<div class="tile-heading"><i class="icon-money"></i> Pendapatan Harian</div>
				<div class="tile-body"><i class="icon-money"></i>
					<h3 class="pull-right">Rp. <?php echo !empty($totalorder['total']) ? $dtFungsi->fuang($totalorder['total']) : 0 ?></h3>
				</div>
				<div class="tile-footer"><a href="<?php echo URL_PROGRAM_ADMIN . 'lap-order?op=view_daily&u_token=' . $u_token ?>" class="link">Selengkapnya</a></div>
			</div>
		</div>
		<?php } ?>
