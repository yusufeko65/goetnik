...
class controllerOrder {
   ...
   public function __construct(){
	...
	public function simpanneworder(){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			foreach($datacart as $dc){
				...
				$tabel .= "<td style=\"text-align:right;margin:0;padding:10px\" align=\"right\" bgcolor=\"#ffffff\">".$this->Fungsi->fFormatuang($dc["hargasatuan"])."</td>";
			...
			if($simpan){
				...
				$alamatpengirim  .= trim($data['alamat_pengirim']).'<br>';
	...
	function hapusdata(){
		...
		$pending	 = $stspending['setting_value'];
		$tgl	   = date('Y-m-d H:i:s');
		$proses    = array();
		if($cek) {
			$pesan =" Anda tidak mempunyai Akses untuk menghapus ";
		} else {
			foreach($dataId as $pesanan_no){
		
				$datast = $this->model->getQytOrder($pesanan_no);
				
				if($datast){
					foreach($datast as $dt){
						if($cekstatus == $pending){
							if($dt['warnaid'] > 0 || $dt['ukuranid'] > 0) {
								$data['stokoptionbertambah'][] = array("pesanan_no"=>$pesanan_no,
																	  "jml"=>$dt['jml'],
																	  "idproduk"=>$dt['produk'],
																	  "idukuran"=>$dt['ukuran'],
																	  "idwarna"=>$dt['warna']
							...
							$data['stokbertambah'][] = array("idproduk"=>$dt['produk'],"jml"=>$dt['jml']);
							//$proses[] = $this->model->updateStok($dt['jml'],$dt['produk']);
						
						}
						$data['hapusorderdetail'][] = array();
				...
				if($cekstatus == $pending) {
					...
					$data['iddata'][] = $reseller;
					$data['tglupdate'][] = $tgl;
					$data['tgl'][] = $tgl;
					$data['totpoin'][] = $poin;
					$data['pelangganid'][] = $reseller;
					$data['nopesan'][] = $data;
					*/
					//$data['updatedeposito'] = 
					...
					//}
					$data['updatepoincust'] = array("idmember"=>'');
				}
			//}
		...
		if(isset($_SESSION['hsadmincart'][$idmember])) {
			...
			if($totalitem > 0){
				foreach($cart as $c){
					...
					$html .= '<td class="text-right">'.$this->Fungsi->fFormatuang($hargasatuan).'</td>';
				...
				$html .= $updt;
				$html .= '<tr><td colspan="5"><td class="text-right">';
   ...
   public function kirimNotifBayar($data) {
	...
	public function cetakInvoicePDF($data,$jenis) {
		...
		if($data['order']['dropship'] == '1') {
			...
			$pdf->Cell(20,7,'Diskon',1,0,'C',1);
			$pdf->Cell(30,7,'Harga',1,0,'R',1);
			...
			foreach($data['detailproduk'] as $dt) {
				...
				$pdf->Cell(30,6,$this->Fungsi->fFormatuang($dt['satuan']),1,0,'R');
