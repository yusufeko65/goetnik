...
class controllerOrder
{
	...
	public function cetakInvoicePDF($data, $jenis)
	{
		...
		$tglorder = isset($data['tanggalBayar'])?strtotime($data['tanggalBayar']):'';
		$tglorder = empty($tglorder)?'-':date('d M Y - H:i',$tglorder);
		...
		$pdf->Cell(200, 4,  $tglorder, 0, 1, 'C');
		$pdf->SetFont('Arial', 'B', 8);

		$pdf->Ln(5);

		$pdf->Line(5, 25, 5 + 200, 25);
	...
	public function cetakInvoice()
	{
		...
		if ($order) {
			...
			$statusOrder = $this->model->getOrderStatus($data['nopesanan']);
			foreach ($statusOrder as $key => $val) {
				if($val['status_id']==10){
					$data['tanggalBayar'] = $val['tanggal'];
				}
			}
	...
	public function cetakLabel()
	{
		...
		*/
		$pdf->SetX(5);
		$y = $pdf->GetY();
		$pdf->MultiCell(69, 4, (int) $order['pesanan_no'] . $order_produk, 0, 'L');
		$pdf->SetX(74);
		$pdf->SetY($y);
		$pdf->setFont('Arial', '', 7);
