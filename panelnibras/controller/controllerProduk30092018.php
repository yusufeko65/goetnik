...
class controllerProduk {
	...
	function editstokoption()
	{
		...
		if($simpan['status'] == 'success') {
			$status = 'success';
			$pesan = 'Berhasil Mengumbah stok';
		} else {
			$status = 'error';
			$pesan = 'Gagal proses Mengubah Stok';
		}
	...
	public function produkAutocomplete(){
		...
		$ukurans = $this->model->getProdukOption($prod['idproduk'],'ukuran');
		$uk = isset($ukurans[0]['id']) ? $ukurans[0]['id']:0;
		$warna = $this->model->getProdukWarnaByUkuran($prod['idproduk'],$uk);
		//$hargadiskon = $this->model->getProdukDiskon($prod['idproduk'],$grupmember);
		$hargadiskon = $this->model->getDiskonProdukByGrup($prod['idproduk'],$grupmember);
		//$dataharga = $this->Fungsi->fcaridata2('_produk_diskon','max(min_beli),harga','idproduk='.$prod['idproduk']);
		//$dataharga = $dtProduk->getProdukDiskons($prod['idproduk'],"-$reseller_bayar");
		$json[] = array(
			...
			/* 'dataharga0' => $dataharga[0],
			'dataharga1' => $dataharga[1]*/
		);
		}
		return $json;
	...
	function getProdukKategori($idproduk)
	{
		...
		if($getkategoriproduk)
		{
			...
			foreach($getkategoriproduk as $kategoriproduk){
				...
				$data[] = array("idkategori" => $kategori['category_id'],
