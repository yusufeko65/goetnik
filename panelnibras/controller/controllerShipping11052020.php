			...
			for ($i=0; $i < $jmldata; $i++) {
				...
				for($x=0;$x < $jmlservis; $x++){
					...
					$dataship[] = array("shipping_code_rajaongkir"=>$kode_ship,
										...
										"servis_id"=>isset($kurir["{$kode_ship}"]["{$datagrab['rajaongkir']['results'][$i]['costs'][$x]['service']}"]["servis"]) ? $kurir["{$kode_ship}"]["{$datagrab['rajaongkir']['results'][$i]['costs'][$x]['service']}"]["servis"] : '',
