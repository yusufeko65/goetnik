...
class modelCustomer {
	...
	function getResellerByID($iddata){
		$sql = "select * from ".$this->tabelnya." LEFT JOIN _customer_deposito on _customer.cust_id = _customer_deposito.cd_cust_id where cust_id='".$iddata."'";
	...
	function getAlamatCustomerByID($id){
		$sql = "select ca_id,ca_cust_id,ca_nama,
		        ca_alamat,ca_propinsi,
				ca_kabupaten,ca_kecamatan,
				ca_kelurahan,ca_kodepos,ca_hp,ca_default
				from _customer_address 
				where ca_id='".$id."'";
