...
class modelProduk {
	...
	{
		$this->db->query("UPDATE _produk_head_warna	SET
						  image_head='".$data['imagewarna']."'
						  WHERE idhead_produk = '".$data['idproduk']."' AND idwarna='".$data['idwarna']."'");
		if(!$this->db->affected_rows()) {
			$sql = $this->db->query("INSERT INTO _produk_head_warna value (null,'".$data['idproduk']."','".$data['idwarna']."','".$data['imagewarna']."')");
			if($sql) {
				$status = 'success';
			} else {
				$status = 'error';
	...
	function updateStokOption($data){
		...
		$sql = "update _produk set jml_stok = ".$data['stok_option']." where idproduk='".$data['idproduk']."'";
	...
	*/
	...
	function getProdukByID($iddata){
		$strsql=$this->db->query("select p.idproduk,p.kode_produk,pd.nama_produk,
							 ...
							 INNER JOIN _produk_deskripsi pd ON p.idproduk = pd.idproduk
							 INNER JOIN _produk_kategori pk ON p.idproduk = pk.idproduk
	...
	function getProdukLimit($batas,$baris,$data){
		...
		$sql = "SELECT _produk.idproduk,_produk.kode_produk,_produk_deskripsi.nama_produk,_produk.jml_stok,_produk.gbr_produk,_produk.hrg_jual 
	...
	function hapusProduk($idproduk){
		...
		*/
		$sql = "delete t1, t2, t3, t4, t5, t6, t7,t8 from ".$this->tabelnya." as t1 
				...
				LEFT JOIN _produk_option_history t7 ON t1.idproduk = t7.idproduk
	...
	public function getOption($id,$warna,$ukuran) {
		$strsql=$this->db->query("select * from _produk_options where idproduk='".$id."' AND warna='".$warna."' AND ukuran='".$ukuran."'");
		return isset($strsql->row) ? $strsql->row : false;
