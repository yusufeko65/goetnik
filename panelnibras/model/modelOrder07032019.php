...
class modelOrder {
	...
	function getOrder($batas,$baris,$data){
		...
		if($data['caridata']!='') $filter[] = " (_order.pesanan_no ='".trim($this->db->escape($data['caridata']))."' OR cust_nama like'".trim($this->db->escape($data['caridata']))."%' OR cust_telp like '".trim($this->db->escape($data['caridata']))."%')";
		...
		$sql = "select idpesanan,_order.pesanan_no,
				...
				pesanan_tgl as tgl,
				status_nama as status,
				pesanan_kurir,dari_poin 
				from _order 
	...
	function getOrderByID($noorder){
		...
		$sql = "select o.pesanan_no,o.pelanggan_id,c.cust_nama,cg.cg_nm as grup_cust,o.pesanan_jml,
				o.pesanan_subtotal,o.pesanan_kurir,o.kurir_perkilo,o.pesanan_tgl,
				...
				p.kelurahan_pengirim,p.kodepos_pengirim,shipping_kode,shipping_kdrajaongkir,servis_code,
	...
	function simpanaddprodukorder($data){
		...
		if(isset($data['updateorderproduk'])) {
			...
			foreach($data['updateorderproduk'] as $options){
				if(	$option['idproduk'] != $data['addprodukorder']['idproduk'] &&
