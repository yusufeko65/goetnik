...
class modelProduk {
	...
	function updateStokOption($data){
		...
		$sql = "update _produk set jml_stok = jml_stok + ".$data['stok']." where idproduk='".$data['idproduk']."'";
	...
	function hapusStokOptionPerProduk($idproduk)
	{
	}
	...
	function getProdukByID($iddata){
		$strsql=$this->db->query("select p.idproduk,p.kode_produk,pd.nama_produk,
							 ...
							 p.hrg_jual,p.status_produk,p.hrg_diskon,p.persen_diskon,
