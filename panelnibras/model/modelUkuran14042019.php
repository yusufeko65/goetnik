...
class modelUkuran {
	...
	function simpanUkuran($data){
		$sql=$this->db->query("insert into ".$this->tabelnya." values (null,'".$data['ukuran_nama']."','".$data['ukuran_alias']."')");
	...
	function editUkuran($data){
		$sql=$this->db->query("update ".$this->tabelnya." set ukuran='".$data['ukuran_nama']."', alias='".$data['ukuran_alias']."' where idukuran='".$data['ukuran_id']."'");
	    $inisial = 'ukuran='.$data['ukuran_id'];
	...
	function getUkuranLimit($batas,$baris,$data){
		...
		$sql = "SELECT idukuran, ukuran 
				FROM _ukuran ".$where." 
				ORDER BY idukuran desc limit $batas,$baris";
