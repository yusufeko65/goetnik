...
class model_Register {
	...
	public function simpanHistory($data) {
	   ...
	   $sql = $this->db->query("INSERT INTO _customer_history values ('','".$data['id']."','".$data['tipe_reseller']."',
	                      '".$data['tglregis']."','1','".$data['ipaddress']."')");
	...
	public function simpanAktivitas($data){
	   $sql = $this->db->query("INSERT INTO _customer_aktivitas set 
	                       ca_cust_id='".$data['id']."',
						   ca_tgl='".$data['tglregis']."',
						   ca_aktivitas='".$data['aktivitas']."',
						   ca_keterangan='".$data['keterangan']."'");
	   
	}
	
	public function simpanInvoice($data) {
	   $sql = $this->db->query("INSERT INTO _reseller_invoice values ('','".$data['id']."','".$data['tipe_reseller']."','".$data['tglregis']."','".$data['biaya_register']."','".$data['stsbayar']."','".$data['keterangan']."')");
