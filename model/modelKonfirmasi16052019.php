...
class model_Konfirmasi {
	...
	public function simpanKonfirm($data){
		...
		$sql = $this->db->query("insert into _order_status values (null,'".$this->db->escape($data['noorder'])."','".$data['tglinput']."','".$data['sts_now']."','".$keterangan."')");
    ...
    public function checkOrder($data){
		$check = $this->db->query("select pesanan_no from _order where pesanan_no='".$this->db->escape($data['noorder'])."' AND status_id = '".$data['sts_pending']."'");
