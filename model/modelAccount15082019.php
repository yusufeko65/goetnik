...
class model_Account {
	...
	public function Simpan($data){
	    $sql = $this->db->query("UPDATE ".$this->tabelnya." set cust_nama='".$data['rnama']."',
	...
	public function simpanHistory($data) {
	   $sql = $this->db->query("INSERT INTO _cust_history values ('','".$data['kode']."',NOW(),'".$data['tipe_reseller']."','1')");
	   if($sql) return true;
	   else false;
	}
	
	public function simpanInvoice($data) {
	   $sql = $this->db->query("INSERT INTO _cust_invoice values ('','".$data['id']."','".$data['tipe_reseller']."',NOW(),'".$data['biaya_register']."','0')");
	   if($sql) return true;
	   else false;
	}
