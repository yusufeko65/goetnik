...
class model_Order
{
	...
	function getOrderByID($noorder)
	{
		...
		$sql = "select o.pesanan_no,o.pelanggan_id,o.pesanan_jml,
				...
				o.no_awb,o.dari_deposito,o.kurir_konfirm,o.keterangan,
	...
	{
		$tabels = array();
		$strsql = $this->db->query("SELECT tanggal,status_nama,keterangan,_status_order.status_id FROM _order_status INNER JOIN _status_order ON
	...
	public function simpanPotonganDeposito($data)

	{
		...
		$sql = "update _order set ". $data['column'] ."='" . $data['jmldeposit'] . "' where pesanan_no='" . $data['pesanan_no'] . "'";
		...
		if ($data['insert_status'] == '1') {
			...
			$sql = "INSERT INTO _order_status set nopesanan='" . $data['pesanan_no'] . "',tanggal='" . date('Y-m-d H:i:s') . "',status_id='" . $data['order_status'] . "',
	                ...
	                keterangan='',admin_id='" . $this->idlogin . "'";
