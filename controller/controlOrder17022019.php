...
class controller_Order {
	...
	public function formEditKurir(){
		...
		return array("status"=>$status,"msg"=>$msg,"order"=>$dataorder,"servis"=>$servis,"totberat" => $totberat);
	}
	...
	public function hapusprodukorder(){
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if($dataorder) {
				...
				if((int)$data['idwarna'] > 0 || (int)$data['idukuran'] > 0){
					$data['stokoptionbertambah'] = array("nopesanan"=>$data['pesanan_no'],
												   "qty"=>$data['qty'],
												   "idukuran"=>$data['idukuran'],
												   "idwarna"=>$data['idwarna'],
												   "idproduk"=>$data['product_id']);
		...
		if($idmember == '' && $idmember == '0') {
			...
			if($dataorder) {
				foreach($dataorder as $order) {
					...
					$json[] = array(
						'noorder' => sprintf('%08s', $order['pesanan_no']),
