   ...
   public function __construct(){
	...
	public function simpanorder(){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['idmember'])) {
			...
			$datashipping = $this->dataShipping->getServisByIdserv($data);
			$data['servis_id'] 	= isset($datashipping['servis_id']) ? $datashipping['servis_id'] : 0;
			$data['shipping']	= isset($datashipping['shipping_kode']) ? $datashipping['shipping_kode'] : '';
			$data['hrgkurir_perkilo'] = isset($datashipping['hrg_perkilo']) ? $datashipping['hrg_perkilo'] : 0;
			...
			$hargaperkilo = $datashipping['hrg_perkilo'];
			$batas = $datashipping['shipping_bataskoma'];
			...
			$checkpoin = $this->dataModel->checkPoin($data['cust_id']);
			...
			if($data['poin'] > 0) {
				if($checkpoin == 0) {
					...
					$sisapoin = (int)$checkpoin - (int) $data['poin'];
					if($sisapoin < 0) {
						$status = 'error';
						$pesan  = "Anda hanya memiliki poin sebesar ".$checkpoin;
					}
				}
				if($status == 'error') {
			...
			foreach($datacart as $dc){	
				//$data["{$key}"] = $value;
				//print_r($key.'<br>');
				$data['orderdetail'][] = array("product_id"=>$dc["product_id"],
									   "qty"=>$dc["qty"],
									   "harga"=>$dc["harga"],
			...
			if($customer['cg_deposito'] == '1') {
				if($customer['cd_deposito'] > 0) {
					if($subtotalbelanja > $customer['cd_deposito'] || $subtotalbelanja == $customer['cd_deposito']) {
						$totatagihan = $subtotalbelanja - $customer['cd_deposito'];
						$data['potdeposito'] = $customer['cd_deposito'];
						
					} else {
						$totaltagihan = 0;
						$data['potdeposito'] = $subtotalbelanja;
					}
					$data['sisadeposito'] = $customer['cd_deposito'] - $data['potdeposito'];
					$data['keterangandeposito'] = "Menggunakan Deposito Pada Order : ".$data['nopesanan'];
			...
			if($simpan){
				...
				*/
				...
				/* kurir */
				$tabel	.= "<tr style=\"margin:0;padding:0\">";
				$tabel .= "<td colspan=\"4\" style=\"text-align:right;margin:0;padding:10px\" align=\"right\" bgcolor=\"#ffffff\"><b>".$datashipping['servis_code']. ' - '.$datashipping['shipping_kode']."</b> </td>";
				...
				if($gunakandeposito == 1) {
					$tabel .= '<tr style=\"margin:0;padding:0\">';
					$tabel .= '<td colspan="3"></td>';
					$tabel .= '<td style=\"text-align:right;margin:0;padding:10px\" align=\"right\" bgcolor=\"#ffffff\"><b>Potongan dari Deposito : </b> </td>';
				...
				$alamatpengirim  = $wilayah_pengirim['provinsi_nama'].', <br>';
				$alamatpengirim  = $wilayah_pengirim['kabupaten_nama'].', <br>';
				...
				$alamatpenerima  = $wilayah_penerima['provinsi_nama'].', <br>';
				$alamatpenerima  = $wilayah_penerima['kabupaten_nama'].', <br>';
				...
				$message   = str_replace("[NAMA PENGIRIM]", $data['nama_pengirim'],$message);
				$message   = str_replace("[ALAMAT PENGIRIM]", $alamatpengirim,$message);
				$message   = str_replace("[NAMA PENERIMA]", $data['nama_penerima'],$message);
	...
	public function keranjangInfo($hcart){
		...
		if($hcart) {
			...
			$html .= '	<div class="col-md-12">
							<a href="'.URL_PROGRAM .'cart" class="btn btn-outline-danger btn-block"><i class="fa fa-shopping-basket" aria-hidden="true"></i> Keranjang Belanja</a>
						</div>
						<br>'; 
