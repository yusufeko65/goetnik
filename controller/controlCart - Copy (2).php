   ...
   public function __construct(){
	...
	public function simpanorder(){
		if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['idmember'])) {
			...
			$datashipping = $modelshipping->getServisByIdserv($data['serviskurir']);
			...
			if($data['poin'] > 0) {
				if($data['poin'] > $subtotalbelanja + $data['potdeposito']) {
					$data['poin'] = $subtotalbelanja + $data['potdeposito'];
				}
			}
			
			$cart['minbeli'] = $customer['min_beli'];
			$cart['syarat'] = $customer['min_beli_syarat'];
			$cart['diskoncust'] = $customer['diskon_grup'];
